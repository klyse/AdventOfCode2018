...
namespace AoC._7
{
	...
	class Program
	{
		...
		static string GetDependencyString(char observedChar)
		{
			var dependentChars = Dependencies.Where(c => c.DependsOnChar == observedChar).Select(c => c.ObservedChar);

			var dependencyList = new List<string>();
			foreach (var dependentChar in dependentChars)
			{
		...
		static void Main()
		{
			...
			Console.WriteLine($"Star 1 Dependency String: {dependencyString}");
